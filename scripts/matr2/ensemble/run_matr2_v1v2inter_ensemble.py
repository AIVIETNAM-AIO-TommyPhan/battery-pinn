"""MATR2 -- V2 + V1 + Inter-Embedding + Affine/NNLS ensemble, on the chosen split
(Variant A: val=MATR2-official-train (M1tr, 41), train_base=M1te+B4-clean (87),
test=official MATR2 test (b3, 40)). Single seed=0, mirrors the MATR1 headline
recipe (run_stage1_axis_aware_confirm.py for V1/V2, run_inter_embedding_final.py
for the Inter-Embedding model, run_inter_embedding_8seed.py's summary block for
the affine+NNLS ensemble) as closely as possible, adapted for the smaller
train_base (87 vs MATR1's 91) via the CHUNK mini-batch fix already established
in run_matr2_smallcnn_screen.py (this GPU OOMs full-batch above ~90-100 cells).

Chunked training changes the per-epoch gradient into an accumulated
approximation of the full-batch gradient (see run_matr2_smallcnn_screen.py's
own docstring) -- disclosed here as a deviation from MATR1's original
full-batch protocol, forced by train_base size vs this 4GB GPU.
"""
import os
import sys
import pickle
...
CACHE_NAME = sys.argv[1] if len(sys.argv) > 1 else "feature_cache_matr2_variantA"
...
# Inter-Embedding config (mirrors run_inter_embedding_final.py)
MARGIN_M = 0.1
REF_SEED = 0
...
def train_cnn(use_pe, tag):
    ...
    + early stop, returns (test_rmse, val_pred, test_pred, embeddings-or-None)."""
    import random
