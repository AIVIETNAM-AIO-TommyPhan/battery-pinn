"""MATR2 (secondary-test / unseen-protocol b3) -- SmallCNN pattern screen.

Single seed=0, no early stopping (patience=infinite) -- purpose is just to see the
train/val/test RMSE pattern over 1000 epochs, printed every 20 epochs, on the new
feature_cache_matr2.pkl split:
  train_base (n=165) = MATR1_train + MATR1_test + MATR34-b4(clean) + HUST
  val (n=40)          = nearest-neighbor RUL match to test (disclosed test-label-informed choice)
  test (n=40)         = official MATR2 test (b3), untouched

Architecture reused as-is from run_stage1_axis_aware_confirm.py's V2 config
(SmallCNNScalarBranchAxisAware, use_pe=False, attention pooling), same top3 scalar
features pre-declared from the MATR1 work (qdlin_diff_std, voltage_slope_50_90,
voltage_soc_90) -- not re-tuned on MATR2.
"""
import os
import sys
import pickle
...
CACHE_NAME = sys.argv[1] if len(sys.argv) > 1 else "feature_cache_matr2"
...
if __name__ == "__main__":
    ...
    # full-batch forward+backward OOMs the 4GB card above ~90-100 cells (train_base
    # here is 87-165 depending on variant).
