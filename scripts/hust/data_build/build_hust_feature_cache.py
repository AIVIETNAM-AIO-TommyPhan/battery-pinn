"""Build feature_cache_hust.pkl for the HUST benchmark.

Official split (HUSTTrainTestSplitter): train=55, test=22 -- no official val.
HUST's train/test RUL distributions are close (train mean=1904.5 std=373.8,
test mean=1887.5 std=419.0 -- see chat 2026-08-31), unlike MATR2's severe
train/test shift, so a plain RUL-tercile-stratified split of the 55 train
cells into train_base/val should track test reasonably well without needing
to borrow external data.

val_frac=0.2 -> val~11, train_base~44 (pre-declared before looking at test).
"""
import os
import sys
import time
...
OUT_PKL = os.path.join(CACHE_DIR, "feature_cache_hust.pkl")

DIFF_BASE = 9
CAP_CLIP_BOUND = 1.05
VAL_FRAC = 20 / 55  # user-requested: HUST train 55 -> train_base 35 / val 20
SPLIT_SEED = 0

TRAIN_IDS_SHORT = ['1-3','1-4','1-5','1-6','1-7','1-8','2-2','2-3',
...
def _load_cells(cell_ids):
    return [BatteryData.load(os.path.join(DATA_ROOT, "HUST", f"HUST_{cid}.pkl")) for cid in cell_ids]
...
def build_pool(cell_ids, tag):
    ...
    print(f"  [{tag}] n={len(cell_ids)}  cnn_feature={tuple(cnn_feature.shape)}  built in {time.time()-t0:.0f}s")
    return cnn_feature, labels, scalar_raw


if __name__ == "__main__":
    annot = LABEL_ANNOTATORS.build({"name": "RULLabelAnnotator"})
    all_cells_dict = {cid: BatteryData.load(os.path.join(DATA_ROOT, "HUST", f"HUST_{cid}.pkl")) for cid in TRAIN_IDS_SHORT}
    labels_all = {cid: float(annot([c]).item()) for cid, c in all_cells_dict.items()}

    order = sorted(TRAIN_IDS_SHORT, key=lambda c: labels_all[c])
    n = len(order)
    tercile = n // 3
    short_pool, mid_pool, long_pool = order[:tercile], order[tercile:2*tercile], order[2*tercile:]
    rng = np.random.RandomState(SPLIT_SEED)
    n_val = round(n * VAL_FRAC)
    n_val_short = round(n_val * len(short_pool) / n)
    n_val_mid = round(n_val * len(mid_pool) / n)
    n_val_long = n_val - n_val_short - n_val_mid
    val_ids = (list(rng.choice(short_pool, size=n_val_short, replace=False)) +
               list(rng.choice(mid_pool, size=n_val_mid, replace=False)) +
               list(rng.choice(long_pool, size=n_val_long, replace=False)))
    train_base_ids = [c for c in TRAIN_IDS_SHORT if c not in val_ids]

    with open("ipynb/exp_v3/data_split/feature_raw_cache.pkl", "rb") as f:
        _, meta = pickle.load(f)
    all_hust = list(meta.index[meta.official_role == "HUST"])
    test_ids = [c.split("_")[-1] for c in all_hust if c.split("_")[-1] not in TRAIN_IDS_SHORT]

    print(f"train_base={len(train_base_ids)}  val={len(val_ids)}  test={len(test_ids)}  (leakage guard: "
          f"{'OK' if not (set(train_base_ids)&set(test_ids) or set(val_ids)&set(test_ids)) else 'FAIL'})")

    train_base_feature, train_base_label, train_base_scalar_raw = build_pool(train_base_ids, "train_base")
    ...
    _scalar_mean = torch.nan_to_num(_scalar_mean, nan=0.0)  # all-NaN column (e.g. qdlin_diff_* -- HUST has no precomputed Qdlin) -> 0
    ...
    C = {
        ...
        "train_ids": train_base_ids, "val_ids": val_ids, "test_ids": test_ids,
    }
