"""HUST -- V2 + V1 + Inter-Embedding + Affine/NNLS ensemble, full Tier3
variant: SOH auxiliary head + monotonicity + Wiener drift/diffusion (SDE),
mirroring run_matr1_v1v2inter_tier3_ensemble.py. RUL loss is plain MSE
(HUST's own established convention, no asymmetric weighting).

Exact loss formulas reused verbatim: lambda_soh=0.01, lambda_mono=0.05,
lambda_wiener=0.05, mono_tolerance=0.002 (all pre-declared, matching MATR1's
Tier3 work -- not re-tuned per dataset). CHUNK-based mini-batch training
(CHUNK=48) since HUST's train pool doesn't fit full-batch on a 4GB GPU.

CLI: python run_hust_v1v2inter_tier3_ensemble.py <cache_name> [seed]
  e.g. python run_hust_v1v2inter_tier3_ensemble.py feature_cache_hust_n2_matr20_withsoh 0
"""
import os
import sys
import pickle
...
OUT_PKL = os.path.join(CACHE_DIR, f"{CACHE_NAME}_v1v2inter_tier3_ensemble_seed{SEED}.pkl")
...
print(f"device={device}  diff_base={DIFF_BASE}  chunk={CHUNK}  seed={SEED}  tier3", flush=True)
