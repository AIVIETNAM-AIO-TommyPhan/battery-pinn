"""HUST -- V2 + V1 + Inter-Embedding + Affine/NNLS ensemble, generic pipeline.

Copy of ../matr1_feature_cache/run_matr2_v1v2inter_ensemble.py, repointed at
this folder's CACHE_DIR so it can run standalone against any
feature_cache_hust*.pkl cache built here (this is what generated
feature_cache_hust_n2_matr20_v1v2inter_ensemble_seed{0..4}.pkl, the source of
the HUST ensemble diagnostics in report_expirement/day_0831/report.md).
Logic is dataset-agnostic. Kept in sync with the MATR2 original in the
sibling folder; if that version's architecture or training loop changes,
port the change here too.

CLI: python run_hust_v1v2inter_ensemble.py <cache_name> [seed]
  e.g. python run_hust_v1v2inter_ensemble.py feature_cache_hust_n2_matr20 0

PINN Tier1 variant: adds an auxiliary SOH-trajectory head (smooth_l1,
lambda_soh=0.01, exact formula from run_v2_pinn_tiers.py's MATR1 work) to
V1/V2's shared backbone. RUL loss here is plain MSE (HUST never needed the
asymmetric under-predict treatment CRUSH did). Requires a *_withsoh.pkl
cache (train_soh/val_soh/test_soh) built by build_hust_soh_cache.py.

CLI: python run_hust_v1v2inter_soh_ensemble.py <cache_name> [seed] [lambda_soh]
  e.g. python run_hust_v1v2inter_soh_ensemble.py feature_cache_hust_n2_matr20_withsoh 0 0.01
"""
import os
import sys
import pickle
...
CACHE_NAME = sys.argv[1] if len(sys.argv) > 1 else "feature_cache_hust_n2_matr20_withsoh"
SEED = int(sys.argv[2]) if len(sys.argv) > 2 else 0
LAMBDA_SOH = float(sys.argv[3]) if len(sys.argv) > 3 else 0.01
FEATURE_CACHE_PKL = os.path.join(CACHE_DIR, f"{CACHE_NAME}.pkl")
OUT_PKL = os.path.join(CACHE_DIR, f"{CACHE_NAME}_v1v2inter_soh{LAMBDA_SOH:.2f}_ensemble_seed{SEED}.pkl")
...
def train_cnn(use_pe, tag):
    ...
    for epoch in range(EPOCHS):
        ...
        for s in range(0, n_train, CHUNK):
            ...
            pred_c, soh_pred_c = model(diffed_c, scalar_c, return_soh=True)
            loss_rul_c = ((pred_c - label_c) ** 2).mean()
            loss_soh_c = nn.functional.smooth_l1_loss(soh_pred_c, soh_c)
            chunk_loss = (loss_rul_c + LAMBDA_SOH * loss_soh_c) * (feat_c.shape[0] / n_train)
            chunk_loss.backward()
            loss_sum += chunk_loss.item()
            del feat_c, label_c, scalar_c, soh_c, diffed_c, pred_c, soh_pred_c, chunk_loss
        optimizer.step()
        if device == "cuda":
            torch.cuda.empty_cache()
