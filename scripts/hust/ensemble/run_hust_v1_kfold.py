"""K-Fold CV for V1 on HUST, with 15 RUL+feature-matched MATR cells fixed in
every fold's train (not rotated -- their role is to help, not to be evaluated).

5-fold split of the 55 official HUST-train cells: each fold trains V1 from
scratch on (44 HUST + 15 MATR), early-stops on its own held-out 11-cell HUST
fold (genuine holdout, never seen in that fold's training -- unlike the
val=train diagnostic earlier in this session, which was NOT a valid
generalization signal). Final test = official 22-cell HUST test, evaluated by
averaging the 5 fold-models' predictions (ensemble), never individually
selected by looking at test.
"""
import os
import sys
import pickle
...
MATR2_CACHE_DIR = os.path.join(REPO, "ipynb", "exp_v3", "matr1_feature_cache")
OUT_PKL = os.path.join(CACHE_DIR, "hust_v1_kfold_seed0.pkl")
...
if __name__ == "__main__":
    with open(f"{CACHE_DIR}/feature_cache_hust.pkl", "rb") as f:
        H = pickle.load(f)
    hust_lookup = {}
    for i, cid in enumerate(H["train_ids"]):
        hust_lookup[cid] = (H["train_base_feature"][i], H["train_base_label"][i], H["train_base_scalar_raw"][i])
    for i, cid in enumerate(H["val_ids"]):
        hust_lookup[cid] = (H["val_feature"][i], H["val_label"][i], H["val_scalar_raw"][i])
    hust_all_train_ids = list(H["train_ids"]) + list(H["val_ids"])  # 55
    test_ids = H["test_ids"]
    for i, cid in enumerate(H["test_ids"]):
        hust_lookup[cid] = (H["test_feature"][i], H["test_label"][i], H["test_scalar_raw"][i])

    with open(f"{MATR2_CACHE_DIR}/feature_cache_matr2.pkl", "rb") as f:
        M = pickle.load(f)
    matr_lookup = {}
    for i, cid in enumerate(M["train_ids"]):
        matr_lookup[cid] = (M["train_base_feature"][i], M["train_base_label"][i], M["train_base_scalar_raw"][i])
    for i, cid in enumerate(M["val_ids"]):
        matr_lookup[cid] = (M["val_feature"][i], M["val_label"][i], M["val_scalar_raw"][i])
    for i, cid in enumerate(M["test_ids"]):
        matr_lookup[cid] = (M["test_feature"][i], M["test_label"][i], M["test_scalar_raw"][i])
    with open("ipynb/exp_v3/data_split/feature_raw_cache.pkl", "rb") as f:
        _, meta = pickle.load(f)
    all_matr = meta.loc[meta['source'].isin(['MATR1', 'MATR34'])]
    lo, hi = 1144, 2659
    matched_matr = [i for i in all_matr.index[(all_matr['rul'] >= lo) & (all_matr['rul'] <= hi)] if i in matr_lookup]
    print(f"HUST pool={len(hust_all_train_ids)}  fixed MATR helpers={len(matched_matr)}  test={len(test_ids)}")

    lookup = {**hust_lookup, **matr_lookup}
    feature_names = H["feature_names"]

    rng = np.random.RandomState(SEED)
    shuffled = list(hust_all_train_ids)
    ...
    if os.path.exists(OUT_PKL):
        ...
        print(f"resuming: folds {list(all_results.keys())} already done")

    for k in range(K_FOLDS):
        if k in all_results:
            print(f"fold{k}: already done, test_RMSE={rmse(all_results[k]['test_pred'], all_results[k]['test_true']):.2f} -- skipping")
            continue
        val_fold = list(folds[k])
        train_fold = [c for c in hust_all_train_ids if c not in val_fold] + matched_matr
        print(f"\n=== Fold {k}: train={len(train_fold)} ({len(train_fold)-len(matched_matr)} HUST + {len(matched_matr)} MATR)  val={len(val_fold)} ===", flush=True)
    ...
    print(f"5-fold ENSEMBLE test_RMSE = {rmse(ens_pred, test_true):.2f}")
