"""CRUSH val v4: same per-source clustering as
build_crush_cluster_val_persource.py, but SNL is forced to K=3 (not
silhouette-auto-picked K=5) with a fixed n_per_cluster=2 (not the 50%-cap
rule) -- per explicit request. Other sources (RWTH, CALCE) keep their
auto-K + 50%-cap treatment from the per-source script; UL_PUR still too
small to cluster (n=3, all -> train_base).
"""
import os
import sys
import pickle
...
for src in sorted(set(src_of)):
    ...
    if src == "SNL":
        km = KMeans(n_clusters=SNL_K, random_state=SPLIT_SEED, n_init=10)
        best_labels = km.fit_predict(combined)
        best_k = SNL_K
        best_sil = silhouette_score(combined, best_labels)
        per_cluster_n = SNL_N_PER_CLUSTER
    else:
        ...
        if best_k is None:
            ...
            continue
        per_cluster_n = None  # use 50%-cap rule below
    ...
    for c in range(best_k):
        ...
        if per_cluster_n is not None:
            take = min(per_cluster_n, len(members))
        else:
...
out_pkl = os.path.join(CACHE_DIR, "feature_cache_crush_cluster_snlk3.pkl")
