"""CRUSH -- V2 + V1 + Inter-Embedding + Affine/NNLS ensemble, generic pipeline.

Copy of ../cruh/run_cruh_v1v2inter_ensemble.py, repointed at this folder's
CACHE_DIR. Logic is dataset-agnostic.

CLI: python run_crush_v1v2inter_ensemble.py <cache_name> [seed]
  e.g. python run_crush_v1v2inter_ensemble.py feature_cache_crush_hustdonors10 0

Asymmetric-loss variant: V2 and Inter-Embedding are both trained with the
same pred<label 2x penalty validated on V1 (run_crush_smallcnn_screen_
asymloss.py, N=10 donors: RMSE 342.18 -> 325.11, beating README=330).
Reuses V1 from the matching *_V1_asymloss_screen_seed<seed>.pkl (not the
symmetric-loss V1 cache) so all three components share the same loss.
"""
import os
import sys
import pickle
...
CACHE_NAME = sys.argv[1] if len(sys.argv) > 1 else "feature_cache_crush_hustdonors10"
SEED = int(sys.argv[2]) if len(sys.argv) > 2 else 0
UNDER_PENALTY = float(sys.argv[3]) if len(sys.argv) > 3 else 2.0  # pre-declared
_pen_tag = f"pen{UNDER_PENALTY:.1f}"
FEATURE_CACHE_PKL = os.path.join(CACHE_DIR, f"{CACHE_NAME}.pkl")
OUT_PKL = os.path.join(CACHE_DIR, f"{CACHE_NAME}_v1v2inter_asymloss_{_pen_tag}_ensemble_seed{SEED}.pkl")
...
def train_cnn(use_pe, tag):
    ...
    for epoch in range(EPOCHS):
        ...
        for s in range(0, n_train, CHUNK):
            ...
            chunk_loss = (weight_c * diff_c ** 2).mean() * (feat_c.shape[0] / n_train)
            chunk_loss.backward()
