"""CRUSH -- V2 + V1 + Inter-Embedding + Affine/NNLS ensemble, PINN Tier1
variant: adds an auxiliary SOH-trajectory head (smooth_l1, lambda_soh=0.01)
to V1/V2's shared backbone, on top of the already-validated asymmetric RUL
loss (pred<label penalty, pre-declared coefficient). Requires a *_withsoh.pkl
cache (train_soh/val_soh/test_soh, cycles 0-99) built by build_crush_soh_cache.py.

Exact Tier1 formula reused from run_v2_pinn_tiers.py (MATR1 PINN work):
    loss_soh = smooth_l1(soh_pred, soh_true)
    loss_total = loss_rul + lambda_soh * loss_soh
loss_rul here is the asymmetric-weighted RUL loss (not plain MSE), since that
component was already confirmed to help CRUSH independently of PINN.

CLI: python run_crush_v1v2inter_soh_ensemble.py <cache_name> [seed] [under_penalty] [lambda_soh]
  e.g. python run_crush_v1v2inter_soh_ensemble.py feature_cache_crush_hust10_calce15_fullval_withsoh 0 1.4 0.01
"""
import os
import sys
import pickle
...
UNDER_PENALTY = float(sys.argv[3]) if len(sys.argv) > 3 else 1.4  # pre-declared (chosen for seed0~seed1 stability)
LAMBDA_SOH = float(sys.argv[4]) if len(sys.argv) > 4 else 0.01  # pre-declared, matches MATR1 Tier1
_pen_tag = f"pen{UNDER_PENALTY:.1f}_soh{LAMBDA_SOH:.2f}"
FEATURE_CACHE_PKL = os.path.join(CACHE_DIR, f"{CACHE_NAME}.pkl")
OUT_PKL = os.path.join(CACHE_DIR, f"{CACHE_NAME}_v1v2inter_{_pen_tag}_ensemble_seed{SEED}.pkl")
...
if __name__ == "__main__":
    ...
    V1_CACHE = os.path.join(CACHE_DIR, f"{CACHE_NAME}_V1_asymloss_screen_seed{SEED}.pkl")
