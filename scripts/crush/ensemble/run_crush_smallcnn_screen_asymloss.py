"""CRUSH -- SmallCNN pattern screen (V1/V2), generic single-model trainer.

Copy of ../hust/run_hust_smallcnn_screen.py, repointed at this folder's
CACHE_DIR. CLI: python run_crush_smallcnn_screen.py <cache_name> [V1|V2] [seed]
  e.g. python run_crush_smallcnn_screen.py feature_cache_crush_cluster_n2 V1 0
"""
import os
import sys
import pickle
...
CACHE_NAME = sys.argv[1] if len(sys.argv) > 1 else "feature_cache_crush_cluster_n2"
...
OUT_PKL = os.path.join(CACHE_DIR, f"{CACHE_NAME}_{_tag}_asymloss_screen_seed{SEED}.pkl")
UNDER_PENALTY = 2.0  # pre-declared, not tuned on test: pred < label costs 2x a symmetric-MSE residual
