"""SNL -- SmallCNN pattern screen (V1/V2), generic single-model trainer.

Copy of ../hust/run_hust_smallcnn_screen.py, repointed at this folder's
CACHE_DIR/feature_cache_snl_split.pkl (built by build_snl_split.py). Logic is
dataset-agnostic -- CACHE_NAME selects which feature_cache_*.pkl to load,
from this folder only. Kept in sync with the HUST/MATR2 originals; if their
architecture or training loop changes, port the change here too.

CLI: python run_snl_smallcnn_screen.py <cache_name> [V1|V2] [seed]
  e.g. python run_snl_smallcnn_screen.py feature_cache_snl_split V2 0
...
CAVEAT (see build_snl_split.py's censoring_note): train_base=11, val=5,
test=15 -- by far the smallest split this project has trained a CNN on.
Treat any result here as a screen, not a claim, regardless of outcome.
"""
import os
import sys
import pickle
...
CACHE_NAME = sys.argv[1] if len(sys.argv) > 1 else "feature_cache_snl_split"
...
CHUNK = 48  # 4GB-GPU-safe forward/backward chunk size (train_base=11 here, so this never actually chunks)
...
if __name__ == "__main__":
    ...
    print(f"n_parameters={n_params}  n_train={train_base_feature.shape[0]}  n_val={val_feature.shape[0]}  n_test={test_feature.shape[0]}", flush=True)
    ...
    with open(OUT_PKL, "wb") as f:
        pickle.dump(dict(history=history, best_val_epoch=best_val_epoch, best_val_rmse=best_val_rmse,
                          n_parameters=n_params, val_test_corr=final_corr, metrics=metrics,
    ...
    print(f"TEST METRICS: RMSE={metrics['rmse']:.2f}  MAE={metrics['mae']:.2f}  MAPE={metrics['mape']:.2f}%  R2={metrics['r2']:.3f}")
