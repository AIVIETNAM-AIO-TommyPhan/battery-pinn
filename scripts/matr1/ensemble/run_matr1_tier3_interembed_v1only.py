"""Leaner version of run_matr1_tier3_interembed_v1embed.py: skips retraining
V2 entirely (its Inter-Embedding baseline is already known from the original
Tier3 5-seed batch -- matr1_v1v2inter_tier3_ensemble_seed{N}.pkl's
inter_test). Only trains V1 (Tier3 loss), extracts its embedding, and runs
Inter-Embedding on it -- then compares directly to the already-saved V2-embed
baseline. Roughly half the compute of the V2+V1 version.

CLI: python run_matr1_tier3_interembed_v1only.py [seed]
"""
import os
import sys
import pickle
...
KNOWN_TIER3_PKL = os.path.join(CACHE_DIR, f"matr1_v1v2inter_tier3_ensemble_seed{SEED}.pkl")
OUT_PKL = os.path.join(CACHE_DIR, f"matr1_tier3_interembed_v1only_seed{SEED}.pkl")

DIFF_BASE = 9
EPOCHS = 1000
EVAL_EVERY = 50
PATIENCE = 200  # matches the original Tier3 seed0-3 V1 protocol (this seed's V1 already used 200 too, except seed4)
...
print(f"device={device}  seed={SEED}  Inter-Embedding on V1-embed only (V2 skipped, reusing known baseline)", flush=True)
...
def train_v1():
    ...
    model = SmallCNNScalarBranchAxisAwareTier3(n_scalar=tr_s.shape[1], use_pe=True).to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        if (epoch + 1) % EVAL_EVERY == 0 or (epoch + 1) == EPOCHS:
            ...
            print(f"  [V1] [{epoch+1:4d}/{EPOCHS}] val_RMSE={val_rmse:7.2f}  test_RMSE={test_rmse:7.2f}"
                  f"{'  <- best val' if improved else ''}", flush=True)
            if device == "cuda":
                torch.cuda.empty_cache()
            if (epoch + 1) - best_val_epoch >= PATIENCE:
                print(f"  [V1] early stop at epoch {epoch+1}", flush=True)
                break
    ...
    print(f"[V1] FINAL test_RMSE={rmse(test_pred, test_true_np):.2f}\n", flush=True)
...
if __name__ == "__main__":
    ...
    # load the already-known V2-embed baseline (no retraining needed)
    with open(KNOWN_TIER3_PKL, "rb") as f:
        known = pickle.load(f)
    known_inter_val_rmse = rmse(known['inter_val'], known['val_true'])
    known_inter_test_rmse = rmse(known['inter_test'], known['test_true'])
    known_v1_test_rmse = rmse(known['v1_test'], known['test_true'])
    print(f"Known (V2-embed) Inter-Embedding baseline: val={known_inter_val_rmse:.2f}  test={known_inter_test_rmse:.2f}")
    print(f"Known V1 solo (sanity target for this run): test={known_v1_test_rmse:.2f}\n")

    print("=== Training V1 (use_pe=True), Tier3 [should reproduce known V1 result above] ===", flush=True)
    v1_val, v1_test, h1_train, h1_val, h1_test = train_v1()

    print("=== Inter-Embedding using V1's embedding (NEW TEST) ===", flush=True)
    interV1_val, interV1_test = run_inter_embedding(h1_train, h1_val, h1_test, train_true, val_true, test_true)
    ...
    print(f"V1 solo:                    test={rmse(v1_test,test_true):.2f}  (sanity check vs known {known_v1_test_rmse:.2f})")
    print(f"Inter (V2-embed, known):    test={known_inter_test_rmse:.2f}")
    print(f"Inter (V1-embed, NEW):      test={rmse(interV1_test,test_true):.2f}")

    with open(OUT_PKL, "wb") as f:
        pickle.dump(dict(
            v1_val=v1_val, v1_test=v1_test,
            interV1_val=interV1_val, interV1_test=interV1_test,
            known_inter_val=known['inter_val'], known_inter_test=known['inter_test'],
