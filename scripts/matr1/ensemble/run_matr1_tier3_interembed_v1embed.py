"""Test: does Inter-Embedding do better using V1's (PE-assisted) embedding
instead of V2's, under Tier3 loss? Motivated by the hypothesis that V2
(no PE) has to overload its embedding with implicit positional inference
under Tier3's temporally-structured extra losses (mono, Wiener), degrading
the embedding Inter-Embedding depends on -- while V1's PE-assisted
embedding may be less corrupted.

Retrains BOTH V1 and V2 with Tier3 loss (deterministic, reproduces the
already-recorded seed's val/test numbers exactly as a sanity gate), extracts
embeddings from BOTH, then runs Inter-Embedding twice: once on V2's h
(baseline, already known), once on V1's h (the new test).

CLI: python run_matr1_tier3_interembed_v1embed.py [seed]
"""
import os
import sys
import pickle
...
OUT_PKL = os.path.join(CACHE_DIR, f"matr1_tier3_interembed_v1embed_seed{SEED}.pkl")
...
print(f"device={device}  seed={SEED}  Inter-Embedding: V1-embed vs V2-embed test", flush=True)
...
def train_cnn(use_pe, tag):
    """Always extracts embeddings this time (both V1 and V2)."""
    import random
    ...
    val_pred = predict_on(val_feature, va_s)
    test_pred = predict_on(test_feature, te_s)
    # always extract embeddings now, regardless of use_pe
...
def run_inter_embedding(h_train, h_val, h_test, train_label_np, val_label_np, test_label_np, tag):
    ...
    print(f"[{tag}] Training {N_DELTA_SEEDS} Inter-Embedding delta-model seeds (K_final={K_final})...", flush=True)
...
if __name__ == "__main__":
    ...
    print("=== Training V2 (use_pe=False), Tier3 [sanity: should reproduce known seed result] ===", flush=True)
    v2_val, v2_test, h2_train, h2_val, h2_test = train_cnn(use_pe=False, tag="V2")

    print("=== Training V1 (use_pe=True), Tier3 [sanity: should reproduce known seed result] ===", flush=True)
    v1_val, v1_test, h1_train, h1_val, h1_test = train_cnn(use_pe=True, tag="V1")

    print("=== Inter-Embedding using V2's embedding (baseline, already known) ===", flush=True)
    interV2_val, interV2_test = run_inter_embedding(h2_train, h2_val, h2_test, train_true, val_true, test_true, tag="Inter-on-V2")
    print(f"Inter(V2-embed): val_RMSE={rmse(interV2_val, val_true):.2f}  test_RMSE={rmse(interV2_test, test_true):.2f}\n")

    print("=== Inter-Embedding using V1's embedding (NEW TEST) ===", flush=True)
    interV1_val, interV1_test = run_inter_embedding(h1_train, h1_val, h1_test, train_true, val_true, test_true, tag="Inter-on-V1")
    print(f"Inter(V1-embed): val_RMSE={rmse(interV1_val, val_true):.2f}  test_RMSE={rmse(interV1_test, test_true):.2f}\n")

    print(f"=== SUMMARY (seed={SEED}) ===")
    print(f"V2 solo:          test={rmse(v2_test,test_true):.2f}")
    print(f"V1 solo:          test={rmse(v1_test,test_true):.2f}")
    print(f"Inter (V2-embed): test={rmse(interV2_test,test_true):.2f}  (should match already-recorded seed{SEED} Inter result)")
    print(f"Inter (V1-embed): test={rmse(interV1_test,test_true):.2f}  <-- NEW")

    with open(OUT_PKL, "wb") as f:
        pickle.dump(dict(
            v2_val=v2_val, v2_test=v2_test, v1_val=v1_val, v1_test=v1_test,
            interV2_val=interV2_val, interV2_test=interV2_test,
            interV1_val=interV1_val, interV1_test=interV1_test,
