"""Ablation: does swapping V2's top3 scalar branch for a SINGLE feature (min or l2)
change performance? This mirrors the original single-feature ablation methodology
(variant_c_feature_cache_ablation_v1.ipynb Section B: SmallCNNScalarBranch(n_scalar=1)
trained on exactly one column) but reruns it on V2 -- the best architecture found this
session (axis-aware attention pooling, no PE) -- instead of the original
SmallCNNScalarBranch backbone.

Architecture: identical to Stage 1 V2 (SmallCNNScalarBranchAxisAware, use_pe=False,
aggregation="attention"), only n_scalar changes (1 instead of 3) and the scalar
column selected changes. Seed=0 only, same protocol (patience=200, matching V2's
confirmed protocol), compared against V2's top3 result (RMSE=75.32).
"""
import os
import sys
import time
...
OUT_PKL = os.path.join(CACHE_DIR, "v2_single_feature_test.pkl")

DIFF_BASE = 9
EPOCHS = 1000
EVAL_EVERY = 50
PATIENCE = 200  # matches V2's confirmed protocol
SINGLE_FEATURES = {"min": "qdlin_diff_min", "l2": "qdlin_diff_l2"}
...
class V2AttentionOnly(nn.Module):
    """Exact V2 architecture: no PE, attention pooling. n_scalar is variable here."""
...
def run_one(tag, tr_s, va_s, te_s, seed=0):
    import random
...
if __name__ == "__main__":
    ...
    v2_top3_rmse = confirm["V2"][0]["rmse"]
    ...
    for tag, colname in SINGLE_FEATURES.items():
        if tag in results:
            print(f"{tag}: already done, rmse={results[tag]['rmse']:.2f} -- skipping")
            continue
        idx = ALL_FEATURE_NAMES.index(colname)
        tr_s = train_base_scalar[:, [idx]]
        va_s = val_scalar[:, [idx]]
        te_s = test_scalar[:, [idx]]
        print(f"\n=== V2 architecture + single feature '{colname}' ({tag}), seed=0, patience={PATIENCE} ===")
        res = run_one(tag, tr_s, va_s, te_s, seed=0)
        res["feature"] = colname
        results[tag] = res
        with open(OUT_PKL, "wb") as f:
            pickle.dump(results, f)
        print(f"{tag} ('{colname}'): RMSE={res['rmse']:.2f}  MAE={res['mae']:.2f}  R2={res['r2']:.3f}  "
              f"short_bias={res['short_bias']:+.2f}  long_bias={res['long_bias']:+.2f}  best_epoch={res['best_epoch']}")

    print(f"\n=== Summary: V2 architecture, single-feature scalar branch vs top3 (RMSE={v2_top3_rmse:.2f}) ===")
    for tag, colname in SINGLE_FEATURES.items():
        r = results[tag]
        delta = r["rmse"] - v2_top3_rmse
        print(f"{tag:5s} ('{colname}'): RMSE={r['rmse']:.2f}  delta={delta:+.2f}  "
              f"short_bias={r['short_bias']:+.2f}  long_bias={r['long_bias']:+.2f}")
